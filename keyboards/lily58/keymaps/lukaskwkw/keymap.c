...
bool encoder_update_user(uint8_t index, bool clockwise) {
    if (index == 0) {              // Encoder on Master side
        if (IS_LAYER_ON(_LOWER)) { //  On _RAISE layer
            if (clockwise) {
                tap_code(KC_MNXT); // Move wheel right
            } else {
                tap_code(KC_MPRV); // Move wheel left
            }
        } else if (IS_LAYER_ON(_RAISE)) { //  On _RAISE layer
            if (clockwise) {
                tap_code(KC_PPLS); // Move cursor right
            } else {
                tap_code(KC_PMNS); // Move cursor left
            }
        ...
        if (IS_LAYER_ON(_RAISE)) { // On _LOWER layer
            ...
            if (clockwise) {
                tap_code(KC_WH_D); // Move cursor down
            } else {
                tap_code(KC_WH_U); // Move cursor up
            }
        } else { // On other layers
            if (clockwise) {
                tap_code(KC_PGDN); // Move wheel down
            } else {
                tap_code(KC_PGUP); // Move wheel up
            }
        }
    }
    return true;
